...
Graph *graph_new(void) {
    ...
    if (!g->entries) { free(g); return NULL; }
    return g;
}
...
void graph_add(Graph *g, EntryType type, const char *pkg, const char *soname) {
    if (!g || !pkg || !soname) return;
    if (g->count >= g->capacity) {
        int new_capacity = g->capacity * 2;
        GraphEntry *entries = realloc(g->entries,
...
int graph_write(const Graph *g, const char *path) {
    ...
    if (!f) {
        fprintf(stderr, "bogartgraph: cannot write %s: %s\n",
                path, strerror(errno));
        if (fd >= 0) { close(fd); unlink(tmp_path); }
        return -1;
    }
    ...
    if (rename(tmp_path, path) != 0) {
        fprintf(stderr, "bogartgraph: cannot replace %s: %s\n", path, strerror(errno));
        unlink(tmp_path);
        return -1;
    }
    return 0;
