...
/* package origin categories */
typedef enum {
    CAT_TRACK    = 0,  /* normal — query and compare */
    CAT_LFS      = 1,  /* LFS 13.0 base — managed via LFS book */
    CAT_BLFS     = 2,  /* BLFS — beyond LFS, stable, low churn */
    CAT_XORG     = 3,  /* Xorg/X11 stack */
    CAT_HYPRLAND = 4,  /* Hyprland ecosystem */
    CAT_TOOLCHAIN= 5,  /* gcc/glibc/binutils — never touch */
    CAT_FONT     = 6,  /* fonts */
    CAT_PYBUILD  = 7,  /* python build/packaging tools */
    CAT_BUILDTOOL= 8,  /* build-time only tools */
    CAT_CUSTOM   = 9,  /* BogartLinux-specific / not in Arch */
    CAT_NOISE    = 10, /* artifacts, partial names, junk */
} Category;
...
static const CatEntry CATALOG[] = {
    ...
    /* Hyprland Ecosystem — in build order */
    {"udis86",              CAT_HYPRLAND},
...
typedef struct {
    char     porg[MAX_LEN];
    char     name[MAX_LEN];
    char     arch[MAX_LEN];   /* arch repo name */
    char     inst[MAX_LEN];   /* installed version */
    char     got[MAX_LEN];    /* arch version */
    Status   status;
    Category cat;
    Buf      buf;
    CURL    *h;
...
static size_t wcb(void *p, size_t sz, size_t n, void *ud) {
    Buf *b = ud;
    size_t t = sz * n;
    b->data = realloc(b->data, b->size + t + 1);
...
int main(void) {
    ...
    while (fgets(line, sizeof(line), f) && np < MAX_PACKAGES) {
        ...
        pkgs[np].got[0] = '\0';
        pkgs[np].cat    = cat;
        pkgs[np].status = (cat != CAT_TRACK) ? ST_SKIP : ST_NOTFOUND;
    ...
    pclose(f);
    /* ── header ─────────────────────────────────────────────── */
    printf(CYAN "Bogart Linux Package Checker Against Arch Linux Repo\n" NC);
    printf(CYAN "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n" NC);
    
    /* count trackable */
    int n_track = 0;
    for (int i = 0; i < np; i++) if (pkgs[i].cat == CAT_TRACK) n_track++;
    printf(CYAN "Tracking %d / %d packages...\n\n" NC, n_track, np);
    fflush(stdout);

    /* ── curl fetch (tracked only) ──────────────────────────── */
    ...
    while (running || done < n_track) {
        ...
        while ((msg = curl_multi_info_read(multi, &ml))) {
            ...
            active--; done++;
            printf("\r  " CYAN "%d/%d" NC, done, n_track); fflush(stdout);
            /* queue next trackable */
            while (next < np) {
    ...
    for (int i = 0; i < np; i++) {
        if (pkgs[i].status != ST_CURRENT) continue;
    ...
    any = 0;
    for (int i = 0; i < np; i++) {
        if (pkgs[i].status != ST_AHEAD) continue;
    ...
    /* ── NOT FOUND (tracked but Arch doesn't know it) ────────── */
    ...
    /* SKIPPED by category */
    printf("\n" DIM BOLD "━━━ SKIPPED ─────────────────────────────────────────────────\n" NC);
    /* print one line per category that has entries */
    for (int cat = CAT_LFS; cat <= CAT_NOISE; cat++) {
        ...
        /* list package names + versions indented */
        for (int i = 0; i < np; i++) {
    ...
    /* summary */
    printf("\n" CYAN "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n" NC);
    printf("  " RED "%d outdated" NC " | " GREEN "%d current" NC " | " BLUE "%d ahead" NC
