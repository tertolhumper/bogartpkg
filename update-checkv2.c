...
typedef struct { const char *name; const char *repo; const char *api; } GHMap;
static const GHMap GHMAP[] = {
    {"hyprland",            "hyprwm/Hyprland",            NULL},
    {"aquamarine",          "hyprwm/aquamarine",          NULL},
    {"hyprlang",            "hyprwm/hyprlang",            NULL},
    {"hyprcursor",          "hyprwm/hyprcursor",          NULL},
    {"hyprutils",           "hyprwm/hyprutils",           NULL},
    {"hyprwayland-scanner", "hyprwm/hyprwayland-scanner", NULL},
    {"hyprtoolkit",         "hyprwm/hyprtoolkit",         NULL},
    {"hyprgraphics",        "hyprwm/hyprgraphics",        NULL},
    {"hyprpaper",           "hyprwm/hyprpaper",           NULL},
    {"hyprland-guiutils",   "hyprwm/hyprland-guiutils",   NULL},
    {"hyprland-protocols",  "hyprwm/hyprland-protocols",  NULL},
    {"hyprwire",            "hyprwm/hyprwire",            NULL},
    {"iniparser",           "ndevilla/iniparser",         NULL},
    {"tomlplusplus",        "marzer/tomlplusplus",        NULL},
    {"glm",                  "g-truc/glm",                  NULL},
    {"re",             "google/re2",         NULL},
    {NULL, NULL, NULL}
};

const char *gh_api(const char *name) {
    for (int i = 0; GHMAP[i].name; i++)
        if (!strcmp(name, GHMAP[i].name)) return GHMAP[i].api;
    return NULL;
}
...
int main(void) {
    ...
    while (fgets(line, sizeof(line), f) && np < MAX_PACKAGES) {
        ...
        if (cat == CAT_HYPRLAND) {
            ...
            const char *api  = gh_api(name);
            if (repo) {
                if (api)
                    snprintf(pkgs[np].url, sizeof(pkgs[np].url), api, repo);
                else
    ...
    while (running || done < n_track) {
        ...
        while ((msg = curl_multi_info_read(multi, &ml))) {
            ...
            active--; done++;
            printf("\r  " CYAN "Arch   %3d/%-3d" NC, done, n_track); fflush(stdout);
            while (next < np) {
    ...
    while (running || done < n_hypr) {
        ...
        while ((msg = curl_multi_info_read(multi, &ml))) {
            ...
            curl_easy_cleanup(msg->easy_handle);
            active--; done++;
            printf("\r  " MAGENTA "GitHub  %3d/%-3d" NC, done, n_hypr); fflush(stdout);
            while (next < np) {
    ...
    printf("  Hyprland: " RED "%3d outdated" NC " | " GREEN "%3d current" NC " | " BLUE "%3d ahead\n" NC,
            hno, hnc, hna);
