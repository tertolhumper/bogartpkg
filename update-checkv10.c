...
#define MAX_LEN       256
#define MAX_ARCH_PKGS 16384
#define PARALLEL      16
#define CACHE_DIR     "/var/cache/check-updates"
#define MIRROR_CORE   "https://geo.mirror.pkgbuild.com/core/os/x86_64/core.db"
#define MIRROR_EXTRA  "https://geo.mirror.pkgbuild.com/extra/os/x86_64/extra.db"
#define DB_TTL        21600
...
static const BlfsSrc BLFSSRC[] = {
    ...
    {"xorriso",             SRC_SKIP,   "libisoburn",           NULL},
    {NULL, 0, NULL, NULL}
};
...
/* ── Local Arch DB ─────────────────────────────────────────────────────── */
typedef struct { char name[MAX_LEN]; char ver[MAX_LEN]; } ArchPkg;
static ArchPkg  arch_db[MAX_ARCH_PKGS];
static int      arch_db_n = 0;
static size_t file_wcb(void *p, size_t sz, size_t n, void *ud) {
    return fwrite(p, sz, n, (FILE *)ud);
}
static int download_db(const char *url, const char *path) {
    CURL *c = curl_easy_init(); if (!c) return -1;
    FILE *f = fopen(path, "wb"); if (!f) { curl_easy_cleanup(c); return -1; }
    curl_easy_setopt(c, CURLOPT_URL, url);
    curl_easy_setopt(c, CURLOPT_WRITEFUNCTION, file_wcb);
    curl_easy_setopt(c, CURLOPT_WRITEDATA, f);
    curl_easy_setopt(c, CURLOPT_FOLLOWLOCATION, 1L);
    curl_easy_setopt(c, CURLOPT_TIMEOUT, 30L);
    curl_easy_setopt(c, CURLOPT_USERAGENT, "BogartLinux-check-updates/6.0");
    CURLcode res = curl_easy_perform(c);
    fclose(f); curl_easy_cleanup(c);
    return res == CURLE_OK ? 0 : -1;
}
static void parse_db(const char *path) {
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        const char *ep = archive_entry_pathname(entry);
        if (!ep) continue;
        size_t el = strlen(ep);
        if (el < 5 || strcmp(ep + el - 5, "/desc") != 0)
            { archive_read_data_skip(a); continue; }
        char buf[8192]; size_t got = 0; la_ssize_t r;
        while ((r = archive_read_data(a, buf + got, sizeof(buf) - got - 1)) > 0)
            got += (size_t)r;
        buf[got] = '\0';
        char pname[MAX_LEN] = "", pver[MAX_LEN] = "";
        char *p = buf;
        while (*p) {
            if (strncmp(p, "%NAME%\n", 7) == 0) {
                p += 7; char *e = strchr(p, '\n'); if (!e) break;
                size_t l = (size_t)(e - p); if (l >= MAX_LEN) l = MAX_LEN - 1;
                strncpy(pname, p, l); pname[l] = '\0'; p = e + 1;
            } else if (strncmp(p, "%VERSION%\n", 10) == 0) {
                p += 10; char *e = strchr(p, '\n'); if (!e) break;
                size_t l = (size_t)(e - p); if (l >= MAX_LEN) l = MAX_LEN - 1;
                strncpy(pver, p, l); pver[l] = '\0'; p = e + 1;
                /* strip epoch: 1:0.41.0 → 0.41.0 */
                char *colon = strchr(pver, ':');
                if (colon) memmove(pver, colon + 1, strlen(colon));
                /* strip pkgrel: 8.20.0-1 → 8.20.0 */
                char *dash = strrchr(pver, '-');
                if (dash) { int ok = 1; for (char *x = dash+1; *x; x++)
                    if (!isdigit((unsigned char)*x)) { ok = 0; break; }
                    if (ok) *dash = '\0'; }
            } else {
                char *nl = strchr(p, '\n'); p = nl ? nl + 1 : p + strlen(p);
            }
            if (pname[0] && pver[0]) break;
        }
        if (pname[0] && pver[0] && arch_db_n < MAX_ARCH_PKGS) {
            strncpy(arch_db[arch_db_n].name, pname, MAX_LEN - 1);
            strncpy(arch_db[arch_db_n].ver,  pver,  MAX_LEN - 1);
            arch_db_n++;
        }
    }
    archive_read_free(a);
}
static const char *arch_db_lookup(const char *name) {
    for (int i = 0; i < arch_db_n; i++)
        if (!strcmp(arch_db[i].name, name)) return arch_db[i].ver;
    return NULL;
}
/* ── End Local Arch DB ─────────────────────────────────────────────────── */
...
/* parse_gh_tag_ver: /tags → array, optional prefix filter */
...
/* Response format: [{"ref": "refs/tags/go1.26.3", ...}, ...] */
void parse_gh_reftag_ver(const char *json, char *out, size_t outlen, const char *prefix) {
    ...
    while (*p) {
        ...
        /* apply prefix filter */
        if (!prefix || strncmp(candidate, prefix, strlen(prefix)) == 0) {
            /* strip prefix to get bare version for comparison */
...
void ver_clean(const char *in, char *out, size_t n) {
    ...
    /* Strip leading v or n (nv-codec-headers uses n prefix) */
    if ((out[0] == 'v' || out[0] == 'n') && isdigit((unsigned char)out[1]))
        memmove(out, out + 1, strlen(out));
    /* Strip go prefix (golang tags: go1.26.1) */
...
//    printf(CYAN "###################################################\n\n" NC);
    ...
    fflush(stdout);
    /* ── Download Arch DBs if stale, parse into local hashmap ─────────── */
    mkdir(CACHE_DIR, 0755);
    const char *db_paths[] = { CACHE_DIR "/core.db", CACHE_DIR "/extra.db", NULL };
    const char *db_urls[]  = { MIRROR_CORE, MIRROR_EXTRA, NULL };
    for (int i = 0; db_paths[i]; i++) {
        struct stat st;
        int stale = (stat(db_paths[i], &st) != 0 || (time(NULL) - st.st_mtime) >= DB_TTL);
        if (stale) {
            printf("\r" CYAN "Syncing %s..." NC "          ", db_paths[i]); fflush(stdout);
            download_db(db_urls[i], db_paths[i]);
            printf("\r" GREEN "Synced %s    \n" NC, db_paths[i]);
        }
    }
    parse_db(CACHE_DIR "/core.db");
    parse_db(CACHE_DIR "/extra.db");
    /* resolve SRC_ARCH locally — mark queued so curl loop skips them */
    for (int i = 0; i < np; i++) {
        if (pkgs[i].src_type != SRC_ARCH || pkgs[i].status == ST_SKIP) continue;
        const char *v = arch_db_lookup(pkgs[i].arch);
        strncpy(pkgs[i].got, v ? v : "NOT_FOUND", MAX_LEN - 1);
        pkgs[i].queued = 1;
    }
    printf(GREEN "Arch Repo Complete! (local DB)\n" NC);
    curl_global_init(CURL_GLOBAL_ALL);
    CURLM *multi = curl_multi_init();
    curl_multi_setopt(multi, CURLMOPT_MAXCONNECTS, (long)PARALLEL);
    /* Unified fetch loop: GitHub-only (Hyprland + BLFS non-Arch) in parallel */
    int n_total = 0;
    for (int i = 0; i < np; i++) {
        if (pkgs[i].status == ST_SKIP) continue;
        if (pkgs[i].src_type == SRC_ARCH) continue;
        if (pkgs[i].cat == CAT_HYPRLAND || pkgs[i].cat == CAT_BLFS) n_total++;
    }
    int active = 0, done = 0, next = 0;
    for (int i = 0; i < np && active < PARALLEL; i++) {
        if (pkgs[i].queued || pkgs[i].status == ST_SKIP) continue;
        if (pkgs[i].cat != CAT_HYPRLAND && pkgs[i].cat != CAT_BLFS) continue;
        ...
        while ((msg = curl_multi_info_read(multi, &ml))) {
            ...
            while (next < np) {
                if (pkgs[next].queued || pkgs[next].status == ST_SKIP ||
                    (pkgs[next].cat != CAT_HYPRLAND && pkgs[next].cat != CAT_BLFS))
                    { next++; continue; }
    ...
    /* Clean got for display — run ver_clean on all tracked got values */
    for (int i = 0; i < np; i++) {
        if (pkgs[i].got[0] && strcmp(pkgs[i].got, "NOT_FOUND")) {
