...
static const BlfsSrc BLFSSRC[] = {
    ...
    {"xorriso",             SRC_ARCH,   "libisoburn",           NULL},
...
//    printf(CYAN "###################################################\n\n" NC);
    ...
    for (int i = 0; i < np && active < PARALLEL; i++) {
        ...
        curl_easy_setopt(pkgs[i].h, CURLOPT_USERAGENT, "BogartLinux-check-updates/4.0");
    ...
    while (running || done < n_track) {
        ...
        while ((msg = curl_multi_info_read(multi, &ml))) {
            ...
            while (next < np) {
                ...
                curl_easy_setopt(pkgs[next].h, CURLOPT_USERAGENT, "BogartLinux-check-updates/4.0");
    ...
    /* Loop 2: CAT_HYPRLAND → GitHub releases API */
    ...
    while (running || done < n_hypr) {
        ...
        while ((msg = curl_multi_info_read(multi, &ml))) {
            ...
            curl_easy_cleanup(msg->easy_handle);
            active--; done++;
            printf("\r  " CYAN "Hyprland %d/%d" NC, done, n_hypr); fflush(stdout);
            while (next < np) {
    ...
    /* Loop 3: CAT_BLFS → Arch/GitHub/GHTag API*/
    active = 0; done = 0; next = 0;
    for (int i = 0; i < np; i++) pkgs[i].queued = 0;
    for (int i = 0; i < np && active < PARALLEL; i++) {
    ...
    printf("\r  " GREEN "BLFS Github Complete!\n\n" NC);
    /* Version comparison  */
...
//    }
    ...
    printf("  Hyprland: " RED "%d outdated" NC " | " GREEN "%d current" NC " | " BLUE "%d ahead" NC
           " | " CYAN "%d not found" NC " | " DIM "%d skipped\n" NC,
           hno, hnc, hna, hnn, hskip);
    printf("  BLFS:     " RED "%d outdated" NC " | " GREEN "%d current" NC " | " BLUE "%d ahead" NC
           " | " CYAN "%d not found" NC " | " DIM "%d skipped\n\n" NC,
    ...
    for (int i = 0; i < np; i++) {
        ...
        printf("  "YELLOW "%-20s" NC " installed: " RED "%-15s" NC " arch: " GREEN "%s\n" NC,
            pkgs[i].name, pkgs[i].inst, pkgs[i].got);
        any = 1;
    }
    ...
    for (int i = 0; i < np; i++) {
        ...
        printf("  " YELLOW "%-20s" NC " installed: " RED "%-15s" NC " github: " GREEN "%s\n" NC,
            pkgs[i].name, pkgs[i].inst, pkgs[i].got);
        hany = 1;
    }
    int bany = 0;
    for (int i = 0; i < np; i++) {
        ...
        printf("  " YELLOW "%-20s" NC " installed: " RED "%-15s" NC " %s: " GREEN "%s\n" NC,
            pkgs[i].name, pkgs[i].inst, src_label, pkgs[i].got);
        bany = 1;
    }
    printf("\n");
